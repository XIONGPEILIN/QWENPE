...
def main():
    ...
    from diffsynth.models.qwen_image_dit import ATTN_CAPTURE_CONFIG
    ...
    # --- DEBUG: Only process Sample #24 ---
    target_idx = 24
    work_list = [item for item in work_list if item[0] == target_idx]
    print(f"DEBUG: Processing only sample #{target_idx}. List size: {len(work_list)}")
    # --------------------------------------
    ...
    checkpoints = [
        {
            "path": repo_root / "train/Qwen-Image-Edit-2511_lora-rank512-cfg/step-28000.safetensors",
            "name": "2511-cfg-28000"
        }
    ]
    ...
    for ckpt_info in checkpoints:
        ...
        for global_idx, sample in work_list:
            ...
            try:
                ...
                # DEBUG: If lock exists, just proceed (we are forcing execution)
                pass
                ...
                for cfg in cfgs:
                    for alpha in alphas:
                        for ab_cfg in ablation_configs:
                            ...
                            # if (sample_dir / "output.png").exists(): ... continue

                            sample_dir.mkdir(parents=True, exist_ok=True)

                            # Enable Attention Capture
                            ATTN_CAPTURE_CONFIG["enable"] = True
                            ATTN_CAPTURE_CONFIG["save_dir"] = str(sample_dir / "attn_maps")
                            print(f"Attention capture enabled. Saving to {ATTN_CAPTURE_CONFIG['save_dir']}")
