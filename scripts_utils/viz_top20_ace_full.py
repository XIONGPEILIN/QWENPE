...
OUTPUT_IMAGE = 'visualize_bottom20_ace_plus_full_sorted_replace.jpg'
NUM_SAMPLES = 20
CELL_SIZE = 512
...
TARGET_TYPES = {
    "REPLACE": "Replace one object category with another"
}
...
def visualize_top20_ace_full():
    ...
    if 'ace_plus' not in dfs:
        print("Error: ACE+ scores not found.")
        return
    ...
    for item in dataset:
        ...
        if etype in TARGET_TYPES.values():
            fname = os.path.basename(item['edit_image'][0])
            if all(os.path.exists(os.path.join(d, fname)) for d in MODEL_DIRS.values()):
                # Get ACE+ FULL image score for sorting
                ace_row = get_scores(dfs['ace_plus'], fname)
                if ace_row is not None:
                    candidates.append({
                        "item": item,
                        "score": ace_row['siglip2_i'], # Sort by FULL Image Semantic Alignment
                        "fname": fname,
                        "type": "REPLACE"
                    })

    # Sort by ACE+ FULL score ascending (Bottom 20)
    candidates.sort(key=lambda x: x["score"], reverse=False)
    
    samples = candidates[:NUM_SAMPLES]
    print(f"Selected {len(samples)} samples sorted by ACE+ Full Score (ASCENDING). Range: {samples[0]['score']:.4f} - {samples[-1]['score']:.4f}")
    ...
    for i, sample in enumerate(samples):
        ...
        src_path = os.path.join(BASE_DIR, item['edit_image'][0])
        src_img = cv2.imread(src_path)
        if src_img is not None:
            src_img = cv2.resize(src_img, (CELL_SIZE, CELL_SIZE))
            draw_text_with_bg(src_img, f"#{i+1} Original ({sample['type']})", 10, 30, color=(0, 255, 255), thickness=2)
            prompt = item['prompt'].replace("Picture 1 is the image to modify. ", "")
            lines = [prompt[j:j+40] for j in range(0, len(prompt), 40)]
            for k, line in enumerate(lines[:3]):
                draw_text_with_bg(src_img, line, 10, 60 + k*25, font_scale=0.45)
            canvas[y_off:y_off+CELL_SIZE, 0:CELL_SIZE] = src_img

        # --- Col 1: TARGET (GT) ---
        gt_path = os.path.join(BASE_DIR, item['image'])
        ...
        for j, (m_name, m_dir) in enumerate(MODEL_DIRS.items()):
            ...
            if res_img is not None:
                ...
                if m_name in dfs:
                    ...
                    if row is not None:
                        color = (0, 0, 255) if m_name == 'ace_plus' else (0, 255, 0)
                        draw_text_with_bg(res_img, m_name.upper(), 10, 30, color=color, font_scale=0.8, thickness=2)
                        
                        # Full Image Scores (Top Left)
                        draw_text_with_bg(res_img, f"Full S={row['siglip2_i']:.3f}", 10, 65, font_scale=0.6)
                        draw_text_with_bg(res_img, f"Full D={row['dreamsim']:.3f}", 10, 90, font_scale=0.6)
                        
                        # Mask Scores (Bottom with BBox)
                        if bbox:
                            bx, by, bw, bh = bbox
                            cv2.rectangle(res_img, (bx, by), (bx+bw, by+bh), (0, 255, 255), 2)
                            # Put mask scores near the box
                            cx = max(10, bx)
                            cy = min(CELL_SIZE-40, by + bh + 10)
                            draw_text_with_bg(res_img, f"Mask S={row['siglip2_i_mask']:.3f}", cx, cy, font_scale=0.5, bg_color=(50,50,50))
                            draw_text_with_bg(res_img, f"Mask D={row['dreamsim_mask']:.3f}", cx, cy+20, font_scale=0.5, bg_color=(50,50,50))
