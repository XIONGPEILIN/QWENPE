...
OUTPUT_IMAGE = 'visualize_top20_remove_siglip_t.jpg'
NUM_SAMPLES = 20
CELL_SIZE = 512
...
def visualize_remove_top():
    ...
    if 'qwen_w_ste' not in dfs:
        print("Error: Qwen scores not found.")
        return

    # Filter candidates for REMOVE task
    candidates = []
    for item in dataset:
        ...
        if etype in TARGET_TYPES.values():
            ...
            if all(os.path.exists(os.path.join(d, fname)) for d in MODEL_DIRS.values()):
                # Get Qwen SigLIP-T score for sorting
                qwen_row = get_scores(dfs['qwen_w_ste'], fname)
                if qwen_row is not None:
                    candidates.append({
                        "item": item,
                        "score": qwen_row['siglip2_t'], # Sort by Image-Text Alignment
                        "fname": fname,
                        "type": "REMOVE"
                    })

    # Sort by Qwen SigLIP-T score DESCENDING
    candidates.sort(key=lambda x: x["score"], reverse=True)
    
    samples = candidates[:NUM_SAMPLES]
    print(f"Selected {len(samples)} REMOVE samples sorted by SigLIP-T. Range: {samples[0]['score']:.4f} - {samples[-1]['score']:.4f}")
    ...
    for i, sample in enumerate(samples):
        ...
        src_img = cv2.imread(os.path.join(BASE_DIR, item['edit_image'][0]))
        ...
        # --- Col 1: TARGET (GT) ---
        gt_img = cv2.imread(os.path.join(BASE_DIR, item['image']))
        ...
        if mask_img is not None:
            ...
            draw_text_with_bg(mask_vis, "Remove Mask", 10, 30)
            ...
            if res_img is not None:
                ...
                if m_name in dfs:
                    ...
                    if row is not None:
                        ...
                        # SigLIP-T Score (Alignment with Instruction)
                        draw_text_with_bg(res_img, f"SigLIP-T: {row['siglip2_t']:.3f}", 10, 65, font_scale=0.6, color=(255, 200, 0))
                        
                        # Full Image Perception
