...
def rank_bbox_metrics():
    ...
    for csv_file in [f for f in os.listdir(RESULTS_DIR) if f.endswith('.csv') and 'summary' not in f]:
        ...
        for short_name, full_type in target_types.items():
            ...
            if len(sub_df) > 0:
                # BBox Metrics
                entry[f"{short_name}_SigLIP_BBox"] = sub_df.get('siglip2_i_bbox', pd.Series([0])).mean()
                entry[f"{short_name}_DINO_BBox"] = sub_df.get('dino_bbox', pd.Series([0])).mean()
                entry[f"{short_name}_DreamSim_BBox"] = sub_df.get('dreamsim_bbox', pd.Series([0])).mean()
    ...
    # 3. Print Tables
    print_rankings(df_res, "acc", "ACCESSORIES (BBox vs Full)")
    print_rankings(df_res, "cloth", "CLOTHING (BBox vs Full)")
...
def print_rankings(df, prefix, title):
    ...
    view = df[["Model", f"{prefix}_SigLIP_BBox", f"{prefix}_SigLIP_Full"]].sort_values(f"{prefix}_SigLIP_BBox", ascending=False)
    print(view.to_markdown(index=False, floatfmt=".4f"))

    # DINO
    print(f"\n--- {title}: DINO (Structure) [Higher is Better] ---")
    view = df[["Model", f"{prefix}_DINO_BBox", f"{prefix}_DINO_Full"]].sort_values(f"{prefix}_DINO_BBox", ascending=False)
    print(view.to_markdown(index=False, floatfmt=".4f"))

    # DreamSim
    print(f"\n--- {title}: DreamSim (Perceptual Distance) [Lower means 'Less Change'] ---")
    view = df[["Model", f"{prefix}_DreamSim_BBox", f"{prefix}_DreamSim_Full"]].sort_values(f"{prefix}_DreamSim_BBox", ascending=True)
    print(view.to_markdown(index=False, floatfmt=".4f"))
